...
void intiialiseTargets(int numoftargets, struct target *targets, int numofobstacles, struct obstacle *obstacles, int rowSize, int columnSize){
    for (size_t i = 0; i < numoftargets; i++)
{       targets[i].currentX = rand() % rowSize;
        targets[i].currentY = rand() % columnSize;
        for (size_t x = 0; x < numofobstacles; x++)
        {
            while (obstacles[x].X == targets[i].currentX)
            {targets[i].currentX = rand() % rowSize;}

            while (obstacles[x].Y == targets[i].currentY)
            {targets[i].currentY = rand() % columnSize;}
        }
...
void initialiseObstacles(int numofobstacles, struct obstacle *obstacles, int rowSize, int columnSize){
...
int main(int argc, char **argv){
    ...
    if (argc == 9) // Four arguments were typed
    {
    ...
    initialiseObstacles(numofobstacles, obstacles, rowSize, columnSize);
    intiialiseTargets(numoftargets, targets, numofobstacles, obstacles, rowSize, columnSize);
